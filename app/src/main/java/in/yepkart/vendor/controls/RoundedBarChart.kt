package `in`.yepkart.vendor.controls
...
class RoundedBarChart : BarChart {
    ...
    private fun readRadiusAttr(context: Context, attrs: AttributeSet) {
        val a: TypedArray = context.theme.obtainStyledAttributes(attrs, R.styleable.RoundedBarChart, 0, 0)
    ...
    private inner class RoundedBarChartRenderer(chart: BarDataProvider?, animator: ChartAnimator?, viewPortHandler: ViewPortHandler?, private val mRadius: Int) : BarChartRenderer(chart, animator, viewPortHandler) {
        ...
        override fun drawDataSet(c: Canvas, dataSet: IBarDataSet, index: Int) {
            ...
            while (j < buffer.size()) {
                ...
                c.drawRoundRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2], buffer.buffer[j + 3], mRadius.toFloat(), mRadius.toFloat(), mRenderPaint)

                if (drawBorder) {
                    c.drawRoundRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2], buffer.buffer[j + 3], mRadius.toFloat(), mRadius.toFloat(), mBarBorderPaint)
                }
